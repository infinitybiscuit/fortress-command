...
func _on_hud_build_item_selected(btype: String) -> void:
	...
	var hint: String = "Click to place: " + btype.capitalize()
	if btype == "bridge":
		hint = "Click next to platform/bridge to extend"
	elif btype == "ramp":
		hint = "Click next to ramp/bridge/platform to connect"
	hud.update_selection_info(hint)
