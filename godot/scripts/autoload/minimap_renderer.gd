...
func _draw_terrain() -> void:
	...
	var tiles: Array = tilemap_node.get("tiles")
	if tiles.is_empty():
		return
...
func _draw_buildings() -> void:
	...
	var buildings: Array = scene.get("all_buildings")
	if buildings == null:
		return
	...
	for building in buildings:
		...
		var faction: int = building.get("faction", 0)
		var color: Color = _get_faction_color(faction)
		
		# Get building world position and size
		var world_pos: Vector2 = building.get("global_position", Vector2.ZERO)
		var world_w: float = building.get("world_width", 32.0)
		var world_h: float = building.get("world_height", 32.0)
...
func _draw_units() -> void:
	...
	var units: Array = scene.get("all_units")
	if units == null:
		return
	...
	for unit in units:
		...
		var faction: int = unit.get("faction", 0)
		var color: Color = _get_faction_color(faction)
		
		# Get unit world position (feet position for CharacterBody2D)
		var world_pos: Vector2 = unit.get("global_position", Vector2.ZERO)
